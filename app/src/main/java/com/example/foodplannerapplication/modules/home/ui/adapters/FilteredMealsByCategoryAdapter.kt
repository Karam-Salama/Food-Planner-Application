...
class FilteredMealsByCategoryAdapter(
    ...
    override fun onBindViewHolder(holder: CategoryViewHolder, position: Int) {
        ...
        if (currentItem != null) {
            Glide.with(context).load(currentItem.strMealThumb).into(holder.categoryImage)
            holder.categoryTitle.text = currentItem.strMeal
        }
    }
    ...
    class CategoryViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView)  {
        val categoryTitle = itemView.findViewById<TextView>(R.id.tv_mealName)
        val categoryImage = itemView.findViewById<ShapeableImageView>(R.id.si_mealImage)
